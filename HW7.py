...
# Your name: Chloe Emch
# Your student id: 4425 9970
# Your email: emchc@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    for players in data['squad']:
        birth_year = players['dateOfBirth'].split("-")
        ...
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)",(int(players['id']), players['name'], int(position_id), birth_year, players['nationality']))
    conn.commit()
...
def nationality_search(countries, cur, conn):
    for country in countries:
        result = cur.execute('SELECT name, position_id, nationality FROM Players WHERE nationality = ?', (country,))
        nationality_tup = result.fetchall()
    
    return nationality_tup
...
def birthyear_nationality_search(age, country, cur, conn):
    year = int(2023 - age)
    result = cur.execute(f'SELECT name, nationality, birthyear FROM Players WHERE nationality = ? AND birthyear < ?', (country, year))
    birthyear_tup = result.fetchall()

    return birthyear_tup
...
def position_birth_search(position, age, cur, conn):
    year = int(2023 - age)
    result = cur.execute('SELECT Players.name, Positions.position, Players.birthyear FROM Players JOIN Positions ON Players.position_id = Positions.id WHERE Positions.position = ? AND Players.birthyear > ?', (position, year))
    pos_birth_tup = result.fetchall()

    return pos_birth_tup
